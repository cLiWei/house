<template name="zy-search">
	<view class="box1">
		<view class="search">
			...
			<text class="searchIcon search-icon" @click="searchStart()">&#xe616;</text>
		...
		<template v-if="isBlock">
			<view class="s-block" v-if="hList.length > 0">
				<view class="header">
					历史记录
					<image src="../../static/zy-search/delete.svg" class="del" mode="aspectFit" @click="delhistory"></image>
				</view>
				<view class="list">
		...
		methods: {
			searchStart: function() {	//触发搜索
				...
				if (_this.searchText == '') {
					...
					this.$emit('search',_this.searchText)
					uni.getStorage({
						key:'search_cache',
						success(res){
							let list = res.data;
